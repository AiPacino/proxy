...
sys.path.append("lib");
from urlfilter import url_is_similar, url_is_repeat, url_contain_custom_focus, netloc_contain_focuskey
...
def get_all_links(page, baseurl, focuskey = ()): 
    ...
    for url in re.finditer(url_pat, page):
        ...
        """这里我们写的url合成真正的url不是很好,因为会有许多我们无法进行判断"""
        """这里我们会有相对链接，绝对链接"""
        ...
        if not netloc_contain_focuskey(url, focuskey): # 是否含特定域名
            continue
...
def main(netloc, focuskey, log, sourcelog):
    for i in range(10):
        ...
        #这个才是关键，因为put的时候，只能put一个不一样的值
        crawl_queue.put(url)

    #这里其实是等待队列为空的时候，才执行别的操作
    #因为我们添加的是守护进程，意思就是主进程结束，子进程也就跟着结束
    #所以我们这里等待一下
    crawl_queue.join()

if __name__ == "__main__":
    # python crawl.py "http://proxy.mimvp.com/index.php" "mimvp.com" "./log/mimvp.txt" "./log/mimvpsource.txt"
    #main("http://proxy.mimvp.com/index.php", "mimvp.com", "./log/mimvp.txt", "./log/mimvpsource.txt")
    main(*sys.argv[1:])
